...
while not  fin:
    ...
    if j.rect.x >=(soldadoEstatico2.rect.x-200):
        ...
        # todos.add(b)
        # b.disparar=False
        # b.tempdis=random.randrange(150)
    ...
    for p in col_p:
        if j.rect.bottom > p.rect.top:
            ...
            if j.accion==7:
                j.accion=3
        if j.rect.top >= p.rect.bottom:
            j.rect.top = p.rect.bottom
            j.vel_y-=.3
...
#---------------------COLISIONES MOTO--------------------------------------------------------
    ...
    for e in col_balsol4:
            if j.rect.right > e.rect.left or j.rect.bottom > e.rect.top or j.rect.left > e.rect.right:
                j.rect.x = e.rect.x-300
                sonidodolor=pygame.mixer.Sound('sound/ouch.ogg')
                sonidodolor.play()
                j.salud-=1
            if j.rect.top < e.rect.bottom:
                j.rect.top = e.rect.bottom
                j.vel_y=0
    ...
    for b in balas:
        ...
        if b.rect.y < 0:
            balas.remove(b)
            todos.remove(b)
        ls_colbalas5=pygame.sprite.spritecollide(b,finales,False)
    ...
    if j.salud ==0:
        ...
        fin=True
    if final.salud ==0:
        gano=pygame.image.load('images/ganador.png').convert_alpha()
        pantalla.blit(gano,[100,50])
        pygame.display.flip()
        reloj.tick(0.2)
        fin=True
